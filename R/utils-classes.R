...
setMethod(
    ...
    function(object) {
        meta_types <- tibble::tibble(
            vars = colnames(get_cell_metadata(object)),
            var_type = purrr::map_chr(map(get_cell_metadata(object), new_pillar_type), 1),
            num_levels = unlist(map(get_cell_metadata(object), ~ length(unique(.x))))
        )
...
#' @export
setGeneric("get_cell_metadata", function(object) {
    standardGeneric("get_cell_metadata")
})

setMethod(
    "get_cell_metadata", "Seurat",
    function(object) {
        object[[]]
    }
)
...
setMethod(
    "get_cell_metadata", "SingleCellExperiment",
    function(object) {
        colData(object) %>%
            as.data.frame()
    }
