...
chevreulSeurat allows exploratory data analysis of a single cell dataset processed using `Seurat` with the command `minimalSeuratApp()`
...
chevreulSeurat provides visualization of embeddings in PCA, tSNE, and UMAP for scRNAseq data summarized at the gene and transcript level. 
...
chevreulSeurat provides different methods of differential expression analysis that users can choose from to determine differential expression of gene. DE methods included in chevreulSeurat are t-test, Wilcoxon rank-sum test, Likelihood ratio test, and MAST
...
chevreulSeurat makes it simple to subset a dataset consisting of a single batch or batch-integrated dataset. Subsetting can be accomplished either in a graphical setting by lasso-selection from a dimensionally reduced plot or by by specification of a formatted .csv file. 
...
Such regression can be performed for any arbitrary gene set. Some preset gene lists are included in `chevreulSeurat` including cell-cycle, mitochondrial, and apoptosis related gene sets.
...
## Server Mode
If we want work with seurat objects interactively and save the results of our exploratory data analysis we need to operate chevreulSeurat in *server mode*. When operating in this manner it is possible to carry out basic project management tasks including:
...
*server mode* requires access to a few sqlite databases with file path details stored in `~/.cache/chevreulSeurat`. `single_cell_projects.db` and `bigwig_files.db` allowing dynamic loading of seurat objects and bigwig (.bw) sample coverage files for each cell. These are initialized upon package installation using the `creat_project_db` function.

The following sections will give details on this expanded functionality.

### Integrate Projects

While operating chevreulSeurat in server mode, separate sequencing batches can be [integrated](https://www.cell.com/cell/fulltext/S0092-8674(19)30559-8) and compared to validate sample processing steps and exclude technical variation in favor of relevant biological variation. 
...
RNA velocity can be used to forecast the future state of a given cell in a developmental or time-dependent process. chevreulSeurat provides visualization and processing functionality using [velocyto](https://velocyto.org/). Velocity can be represented on UMAP or tSNE plots in multiple formats and exported as needed. 

RNA velocity necessitates the calculation of spliced and unspliced transcript counts as implemented on the command-line in velocyto. As a result integration within chevreulSeurat demands some familiarity with the command-line and the availability of a properly formatted [loom](https://velocyto.org/velocyto.py/tutorial/cli.html#about-the-output-loom-file) file. To enable plotting of RNA velocity, the path to the corresponding loom file must be provided.
